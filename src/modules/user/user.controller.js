...
const createUser = async (req, res) => {
   ...
   const newUser = await userService.createUser({ ...req.body, role: UserRole.STUDENT, status: UserStatus.ACTIVE });
   ...
   try {
      ...
      if (!user) {
         res.status(HttpStatus.NOT_FOUND).json(
            new ErrorResponse(HttpStatus.ITEM_NOT_FOUND, { key: 'userId', message: 'Not found user' })
         )
      }
      if (user.role === UserRole.ADMIN && req.user.role === UserRole.ADMIN) {
         res.status(HttpStatus.FORBIDDEN).json(
            new ErrorResponse(HttpStatus.UNPROCESSABLE_ENTITY, { key: 'userId', message: 'Cannot update another admin' })
         )
      }
...
const deleteUser = async (req, res) => {
   try {
      ...
      return res.status(HttpStatus.OK).json(new SuccessResponse({ id: userId }))
   } catch (error) {
      return res
         .status(HttpStatus.INTERNAL_SERVER_ERROR)
         .json(
...
const getUserByRoomId = async (req, res) => {
   try {
      ...
      return res.status(HttpStatus.OK).json(
         new SuccessResponse({ items: userList, totalItems: userList.length })
      )
   } catch (error) {
      return res
         .status(HttpStatus.INTERNAL_SERVER_ERROR)
